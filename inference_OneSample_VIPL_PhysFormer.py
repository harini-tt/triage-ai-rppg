...
def train_test():
    ...
    model.load_state_dict(torch.load(dict_list[0], map_location=device))

    model.eval()
  
  
    VIPL_testDL = VIPL(VIPL_test_list, VIPL_root_list, transform=transforms.Compose([Normaliztion(), ToTensor()]))
    dataloader_test = DataLoader(VIPL_testDL, batch_size=1, shuffle=False, num_workers=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_data', type=str, default="./") # Changed to local directory
